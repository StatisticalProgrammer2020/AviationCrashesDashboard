...
shinyServer(function(input, output, session) {
  ...
  # function that returns a dataset filtered by year and flight type in the first page
  ...
  # function that returns a dataset filtered by year and crash cause in the second page
  ...
  # function that returns a dataset filtered by year and operator in the third page
  ...
  # function that returns a dataset filtered by year and aircraft in the fourth page
  ...
  # function to compute for the deadliest crash stat in the first page  
  ...
  # function to compute for the crash survival rate in the first page  
  ...
  # function to compute for the common crash reason in the first page  
  ...
  # function to modify all the plots in the second page
  modify_crashplots <- reactive({
    
    yearly_table <- adjusted_crashcauses()
    
    sma_est = length(unique(yearly_table$Year)) # for generating the trendline in the bar chart
    
    output$crashtrend <- renderPlot({ # creates the bar trend of crashes
    ...
    output$survival_dist <- renderPlot({ # creates the pie chart of crashes with survivors
    ...
    output$fatalities_dist <- renderPlot({ # creates a bar chart of passengers by outcome
      ...
      # creates a new table combining crew/passenger columns with their respective outcomes
      ...
      # adds another data point in case the user wants to select "all" of the crash causes
    ...
    output$countries_bar <- renderPlot({ # creates a bar chart of top countries
      ...
      # renames long named/more frequent countries
    ...
    output$terrain_bar <- renderPlot({ # creates a bar chart of top terrains
  ...
  # function to modify all the plots in the third page
  modify_crash_operator_plots <- reactive({
    ...
    output$crashoperatortrend <- renderPlot({ # creates the bar trend of crashes
    ...
    output$planes_dist <- renderPlot({ # creates the bar chart of most crashed aircrafts
    ...
    output$countries_operator_bar <- renderPlot({  # creates the bar chart of most crashed countries
    ...
    output$terrain_operator_bar <- renderPlot({  # creates the bar chart of most crashed terrains
      
      top_terrains <- yearly_table %>% group_by(`Crash site`) %>% summarize(Freq = n())
      top_terrains <- top_terrains %>% mutate(perc = round(100*Freq/sum(top_terrains$Freq)))
      top_terrains <- top_terrains %>% arrange(-Freq) 
      
      # renames long-named reasons
      top_terrains$`Crash site`[top_terrains$`Crash site` == "Airport (less than 10 km from airport)"] <- "Near airport"
      top_terrains$`Crash site`[top_terrains$`Crash site` == "Lake, Sea, Ocean, River"] <- "Waters"
      top_terrains$`Crash site`[is.na(top_terrains$`Crash site`)] <- "Unknown"
    ...
    output$cause_operator_dist <- renderPlot({  # creates the bar chart of top reasons for crashes
  ...
  # function to modify all the plots in the fourth page
  modify_crash_aircraft_plots <- reactive({
    
    yearly_table <- adjusted_crashaircraft()
    
    sma_est = length(unique(yearly_table$Year)) # for generating trendline in bar chart
    
    output$crashaircrafttrend <- renderPlot({  # creates the bar trend of crashes
    ...
    output$cause_aircraft_dist <- renderPlot({  # creates the bar chart for the top reasons of crashes
    ...
    output$phases_dist <- renderPlot({  # creates the pie chart of most common aircraft phases for crashes
      ...
      # renames long-named flight phases
    ...
    output$ftype_dist <- renderPlot({  # creates the bar chart of most common flight types by crashes
      ...
      # renames long-named flight type
    ...
    output$countries_aircraft_bar <- renderPlot({  # creates the bar chart of top countries by crashes
      
      top_countries <- yearly_table %>% filter(Country != "World") %>% group_by(Country) %>% summarize(Freq = n()) 
      top_countries <- top_countries %>% mutate(perc = round(100*Freq/sum(top_countries$Freq))) 
      top_countries <- top_countries %>% arrange(-Freq) %>% head(7)
      
      # renames long-named countries
      top_countries$Country[top_countries$Country == "United States of America"] <- "USA"
      top_countries$Country[top_countries$Country == "United Kingdom"] <- "UK"
  ...
  # first headline stat output in the first page
  ...
  # second headline stat output in the first page
  ...
  # third headline stat output in the first page
  ...
  # fourth headline stat output in the first page
  ...
  # for navigating through diff tabs in the fifth page
  output$tabset1Selected <- renderText({
    input$tabset1
  })
  
  # function to output the updated table in the fifth page based on inputs from filters
  ...
  # function to create a dataset for updating the map of crashes
  update_chart <- reactive({
    df <- adjusted_crashes() %>% group_by(Country) %>% summarize(count = n())
    colnames(df)[1] <- "id"
    
    # for dynamic labelling of the map
  ...
  # function to print the map based on the inputs
  ...
  # function to print the table in the first page beside the map
  ...
  # updates plots when the cause filter is modified in the second page
  ...
  # updates plots when the year filter is modified in the second page
  ...
  # updates plots when the operator filter is modified in the third page
  ...
  # updates plots when the year filter is modified in the third page
  ...
  # updates plots when the aircraft filter is modified in the fourth page
  ...
  # updates plots when the year filter is modified in the fourth page
  ...
  # updates the table in fifth page when the confirmation button is clicked
  observeEvent(input$id11,{
    
        # divides table into five sections
        ...
        # prints 12 results in each table
